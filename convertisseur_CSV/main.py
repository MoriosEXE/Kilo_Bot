...
def data_read(name,type_data = True):
    data_by_time = [["step","ID","pos_X","pos_Y","Angle","Opinion","Puissance"]]
    data_by_robot = []
        ...
        if raw_data != "":
            ...
            for bot in brut_line:
                if "ID" in bot:
                    ...
                    data_by_time.append([step,id_bot, pos_x, pos_y, angle,opinion,puissance])
...
nom = "test_nb_rencotre_t1"
data = data_read(nom)

analyse = [["step", "opinion_1", "somme_1", "opinion_2", "somme_2","puissance_1","puissance_2"]]
...
for i in data[1:]:
    if (actual_step != int(i[0])):
        ...
        value = (line[2],line[4],line[5],line[6])
        line = [actual_step,10,value[0],11,value[1],value[2]/value[0],value[3]/value[1]]
        ...
        if(int(i[5]) == 10 ):
            value = (line[2]+1,line[4],int(i[6]) + line[5],line[6])
        else:
            value = (line[2] , line[4]+1,line[5], line[6]+ int(i[6]))

        line = [actual_step,10,value[0],11,value[1],value[2],value[3]]


my_ods = OrderedDict() # from collections import OrderedDict
my_ods.update({"donnée_brut": data})
my_ods.update({"analyse": analyse})
save_data(f"{nom}.ods", my_ods)